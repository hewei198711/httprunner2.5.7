...
class ResponseObject(object):
    ...
    def __init__(self, resp_obj):
        """ initialize with a requests.Response object
            用requests.Response初始化对象

        Args:
            resp_obj (instance): requests.Response instance请求.响应实例
    ...
    def _extract_field_with_jsonpath(self, field):
        ...
        For example, response body like below:例如，响应体如下所示
        {
            "code": 200,
            "data": {
                "items": [{
    ...
    def _extract_field_with_regex(self, field):
        """ extract field from response content with regex.
            ...
            requests.Response主体可以是json或html文本

        Args:
            field (str): regex string that matched r".*\(.*\).*"

        Returns:
            str: matched content.匹配的内容
    ...
    def _extract_field_with_delimiter(self, field):
        ...
        Args:
            field (str): string joined by delimiter.由分隔符连接的字符串
            e.g.
                "status_code"
                "headers"
                "cookies"
